...
| スキル | サマリ | 依存 | ステータス |
|---|---|---|---|
| **AI Buildout** (`ai-buildout`) | Map the AI-datacenter buildout value chain to tickers, find the binding layer that is heating up, track hyperscaler capex (the leading indicator), and grade whether the buildout thesis is still intact. | `fmp` **required**, `unusual_whales` optional, `alpaca` optional | production |
...
| **Mover Screener** (`mover-screener`) | Hunt high-beta secondary "mover" stocks market-wide for an options campaign, bucketing Stage 2 uptrends as CALL and Stage 4 breakdowns as PUT candidates, each pre-checked by an options liquidity gate. | `finviz` _recommended_, `fmp` **required**, `unusual_whales` optional, `alpaca` optional | production |
...
| スキル | サマリ | 依存 | ステータス |
|---|---|---|---|
| **Performance Tracker** (`performance-tracker`) | Turn an Alpaca account and thesis bookkeeping into quantstats tearsheets — an account-level report vs SPY and a strategy-level trade ledger that measures return on deployed capital (per-trade R) instead of the cash-diluted account. | `alpaca` **required**, `thesis_files` optional | production |
